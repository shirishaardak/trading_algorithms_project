...
df['Close-diff'] = df['Close'] - df['Close'].shift(11)
df["Line+100"] = 100
df["Line-100"] = -100
...
orders.to_csv(r'D:/trading_algorithms_project/report_data/close_different_21_days_paper.csv',)
...
fig.add_trace(go.Scatter(x=df.index, y=df['Close-diff']), row=2, col=1)
fig.add_trace(go.Scatter(x=df.index, y=df["Line+100"]), row=2, col=1)
fig.add_trace(go.Scatter(x=df.index, y=df["Line-100"]), row=2, col=1)
