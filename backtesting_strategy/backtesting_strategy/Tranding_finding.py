...
for i in range(len(df)):
    if df['HA_close'][i] == df['max_high'][i]:
        trand = df['HA_high'][i-1]
        df['trand-line'][i] = df['HA_high'][i-1]
    elif df['HA_close'][i] == df['max_low'][i]:
        trand = df['HA_low'][i-1]
        df['trand-line'][i] = df['HA_low'][i-1]    
    else:
...
fig.add_trace(go.Scatter(mode='markers', x=df.iloc[max_idx].index, y=df.iloc[max_idx]['HA_high'], marker=dict(color='Blue',size=12,), name="Buy"), row=1, col=1)
