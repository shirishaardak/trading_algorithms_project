...
# ------ Create indicator ------ #
df['Close-s'] = signal.savgol_filter(df.Close, 49, 5)
df['Ema'] = ta.ema(close=df['Close'], length=42)
max_idx = argrelextrema(df['Close-s'].values, np.greater_equal, order=11)[0]
min_idx = argrelextrema(df['Close-s'].values, np.less_equal, order=11)[0]
...
fig.add_trace(go.Scatter(x=df.index, y=df['Close-s']),)
...
#fig.add_trace(go.Scatter(x=df.index[low_high], y=df['Close'].iloc[low_high]),)
